class P {
    ...
    static plugins: { [key: string]: _Plugin } = {};

    static init(selector: string = '.js-l') {
        P.selector = selector;
        P.initComponents(window.document)
    }

    static plugin(name: string, plugin: _Plugin) {
        P.plugins[name] = plugin
    }

    static initComponents(dom: Element | Document) {
        let items = dom.querySelectorAll(P.selector);
...
class Component {
    ...
    constructor(public el: Element) {
        ...
        if (P.plugins.hasOwnProperty(this.selector)) {
            let ret = P.plugins[this.selector].init.call(P.plugins[this.selector], el);
        }
    }

    /*constructor(public name: string, public el: Element, public template: string) {
        // let's eval script
        if (P.plugins.hasOwnProperty(name)) {
            let render = function (template: string, data?: any) {
                // todo: mustache rendering
                return template;
            ...
            let ret = P.plugins[name].init.call(MVC.plugins[name], el);
            if (ret != null && typeof ret === 'function') {
                html = ret.call(ret, render, template)
            } else {
                html = template;
            }
